page 50127 "Res. From Picture Wizard"
{
    PageType = NavigatePage;
    Caption = 'Set up resource from picture analysis';
        ...
        area(content)
        {
            ...
            group(FirstPage)
            {
                ...
                group("Welcome to the Wizard")
                {
                    ...
                    {
                        ApplicationArea = All;
                        Caption = 'New from Picture uses the Computer Vision API from Azure Cognitive Services to match the images you upload in the New from Picture page with resource categories. This makes it easy and fast for you to create resources when you have their pictures available.';
                    }
            ...
            group(FinalPage)
            {
                ...
                group("That's it!")
                {
                    ...
                    {
                        ApplicationArea = All;
                        Caption = 'Next time you upload a resource image in the New from Picture action, we will analyze it for you. You can opt out at any time from the Image Analysis Setup page.';
                    }
                    label(ChooseFinish2)
                    {
                        ApplicationArea = All;
    ...
    actions
    {
        area(processing)
        {
            ...
            action(ActionFinishAndEnable)
            {
                ...
                begin
                    Session.LogMessage('0000JYV', WizardAcceptedTelemetryMsg, Verbosity::Normal, DataClassification::SystemMetadata, TelemetryScope::ExtensionPublisher, 'Category', ResFromPicture.GetTelemetryCategory());
                    ResFromPicture.EnableImageAnalysisScenario();
                    CurrPage.Close();
                end;
            }
        }
