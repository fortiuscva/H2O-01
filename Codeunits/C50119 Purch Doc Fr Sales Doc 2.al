codeunit 50119 "Purch. Doc. Fr. Sales Doc 2"
{
    ...
    var
        ...
        PurchOrderFromSalesInvoice: Page "Purch. Order From Sales Inv";
        NoFilter: Text;
    begin
        ...
        //add code to for sales invoice >>
        IF SalesHeader."Document Type" = SalesHeader."Document Type"::Invoice then begin
            PurchOrderFromSalesInvoice.LookupMode(true);
            PurchOrderFromSalesInvoice.SetSalesInvoiceNo(SalesHeader."No.");
            if PurchOrderFromSalesInvoice.RunModal() <> ACTION::LookupOK then begin
                OrderPlanningMgt.PrepareRequisitionRecord(RequisitionLine);
                exit;
            end;
        end;
    ...
    begin
        IF SelectedSalesLine.Type = SelectedSalesLine.Type::Item then begin
            SalesLine.Copy(SelectedSalesLine);
        ...
        IF SelectedSalesLine.Type = SelectedSalesLine.Type::Resource then begin
            SalesLine.Copy(SelectedSalesLine);
            ...
            if SalesLine.FindSet() then begin
                ...
                VendorNo := Res."Vendor No.";
                DefaultVendorFound := (VendorNo <> '');

                while DefaultVendorFound and (SalesLine.Next() <> 0) do begin
                    Res.Get(SalesLine."No.");
                    DefaultVendorFound := (VendorNo = Res."Vendor No.");
                end;
